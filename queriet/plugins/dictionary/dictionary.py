...
class DictionaryInfoPanel(PluginHandler.PluginPanel):
    ...
    def __init__(self, parent):
        ...
        self.choices = [
            ...
            'hyphenation']
        self.choicedict = {
            self.choices[0]: defin,
            self.choices[1]: syno,
            self.choices[2]: anton,
            self.choices[3]: partos,
            self.choices[4]: usagex,
            self.choices[5]: pronun,
            self.choices[6]: hyphen}
        self.InputPanel = defaultPanels.InputPanelWithRadiobox(
            self,
            self.Search,
            self.choices,
            InputFieldString='Dictionary lookup',
            EnterFunc=self.Search,
            autohide=False)
        self.OutputPanel = defaultPanels.ReadOnlyOutputPanel(
            self, autohide=False)
    ...
    def Search(self, event):
        ...
        if self.InputPanel.input.GetValue() is None or self.InputPanel.input.GetValue() == "":
...
def GetResultString(vtype, term):
    ...
    if isinstance(vtype['returntype'], list):
        for item in res:
            # All of these use the same general format
            if vtype['name'] == 'definition' or vtype[
                    'name'] == 'synonym' or vtype['name'] == 'usage example':
        ...
        if vtype['name'] == 'antonym':
            # only one attribute in the return dict if we are looking for
            # antonyms, the text attribute is a list of them though
            count = 0
