...
def define(term):
	...
	try:
		...
		raise UrbanDictionaryParseError("Error parsing json response. %s" %(e))
	try:
		results = parse(urbjson)
	except KeyError as e:
		raise UrbanDictionaryResultParseError("Error parsing result: %s." %(e))
	return results
...
def parse(urbjson):
	...
	for entry in urbjson['list']:
		itm = UrbanDictionaryResult(entry['word'], entry['definition'], entry['permalink'], entry['defid'], entry['thumbs_up'], entry['thumbs_down'], entry['author'], entry['example'])
		res.append(itm)
	return res
...
class UrbanDictionaryResult(object):
	...
	def __init__(self, term, definition, permalink, id, thumbs_up, thumbs_down, author, example=None):
		self.term = term
...
class UrbanDictionaryParseError(Exception): pass
class UrbanDictionaryResultParseError(Exception): pass