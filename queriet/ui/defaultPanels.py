...
# Also, they don't have OnGain / OnLoseFocus methods, these are building
# blocks and as such your plugin's focus methods are expected to clear the
# fields manually.
...
class StandardInputPanel(wx.Panel):
    ...
    def __init__(
            ...
            parent,
            SearchFunc,
            InputFieldString='Search &term or equation',
        ...
        # A sizer for items in the input panel
        self.inputSizer = wx.BoxSizer(wx.HORIZONTAL)
        self.inputSizer.Add(self.inputStatic, 1, wx.TOP |
                            wx.LEFT | wx.BOTTOM, 5)  # adding our input label
        self.inputSizer.Add(self.input, 3, wx.TOP | wx.BOTTOM, 10)
        self.inputSizer.Add(self.SearchButton, 2, wx.TOP |
                            wx.RIGHT | wx.BOTTOM, 20)
...
class InputPanelWithRadiobox(wx.Panel):
    ...
    def __init__(
        ...
        self.Radiobox = wx.RadioBox(
            self, id=-1, choices=RadioboxChoices, style=wx.RA_VERTICAL)
...
class ReadOnlyOutputPanel(wx.Panel):
    ...
    def __init__(self, parent, autohide=True):
        ...
        self.output = wx.TextCtrl(
            self, -1, size=(500, 500), style=wx.TE_MULTILINE | wx.TE_READONLY)
