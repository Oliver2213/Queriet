...
class MainUI(wx.Frame):
    ...
    def __init__(self, controller, parent, title):
        ...
        self.CreateIcon()
        self.Center()
        if self.controller.config['general']['open_interface_on_startup']:
            self.Show()
        else:
            self.icon.ShowBalloon(
                "Queriet running!",
                "Queriet is now running in the background, awaiting your use.",
                3000)
    ...
    def setup(self):
        ...
        # A list to hold the different APIs
        self.ListSizer = wx.BoxSizer(wx.HORIZONTAL)
        self.MainSizer = wx.BoxSizer(wx.HORIZONTAL)
        self.CurrentPlugin = None
        self.CurrentPluginNumber = -1
        self.InfoPanel = None
        # the main pannel that children pannels inherit from
        self.panel = wx.Panel(self)

        # API list
        self.ListPanel = wx.Panel(self.panel)

        self.apiStatic = wx.StaticText(
            self.ListPanel, -1, 'API')  # A label for our listview
    ...
    def SetSizers(self):
        ...
        # adding this label to our api sizer
    ...
    def SetupMenuBar(self):
        ...
        try:
            ...
            self.MenuBar_file_options = utils.CreateMenuItem(
                self.MenuBar_file, "&Options...", self.ShowOptions)
            self.MenuBar_file_hide = utils.CreateMenuItem(
                self.MenuBar_file, 'Close to &tray', self.showhide)
            self.MenuBar_file_exit = utils.CreateMenuItem(
                self.MenuBar_file, 'E&xit', self.controller.Close, id=wx.ID_EXIT)

            self.MenuBar.Append(self.MenuBar_file, '&File')
            self.MenuBar_help = wx.Menu()
            self.MenuBar_help_about = utils.CreateMenuItem(
                self.MenuBar_help, "&About...", self.OnAbout, id=wx.ID_ABOUT)
            self.MenuBar_help_OpenSite = utils.CreateMenuItem(
                self.MenuBar_help, 'Open Queriet &site...', self.OpenSite)
            self.MenuBar_help_OpenIssuePage = utils.CreateMenuItem(
                self.MenuBar_help, 'File an &issue...', self.OpenIssuePage)
    ...
    def SetInfoPanel(self, info):
        ...
        if self.InfoPanel:
            self.log.debug("Existing info panel %s, hiding it." %
                           (self.InfoPanel))
            self.InfoPanel.Hide()
        self.InfoPanel = info
        self.SetSizers()
    ...
    def ChangeFocus(self, value):
        ...
        if self.CurrentPlugin:
            try:
                ...
                self.log.exception(
                    "Error while running OnLoseFocus method of plugin!")
        ...
        try:
            ...
            self.log.exception(
                "Error while running OnGainFocus method of plugin!")

    def ShowOptions(self, e):
        """Display the options dialog"""
        self.log.debug("Getting options dialog.")
        DLG = Options(self.controller.config, parent=None,
                      title="Queriet options")
    ...
    def OnListChange(self, event):
        ...
        if sel < 0:
            return
        self.log.debug(
            "Plugin / API list selection changed.\n%s is now selected." %
            (sel))
    ...
    def OnClose(self, event):
        ...
        # We haven't asked the user what they want to do yet, let's do so
        if self.controller.config['dialogs']['exit_action'] == False:
            self.log.debug(
                "The user hasn't been asked about exiting to the background. Asking now...")
            dlg = wx.MessageDialog(
                self,
            ...
            res = dlg.ShowModal()
            self.log.debug(
                "User said yes, run in the background: %s. User canceled close: %s." %
                (res == wx.ID_YES, res == wx.ID_CANCEL))
            dlg.Destroy()
            if res == wx.ID_YES:
                ...
                return
        if self.controller.config['general']['exit_to_tray']:
...
class SystemTrayIcon(wx.TaskBarIcon):
    ...
    def CreateMenu(self):
        ...
        try:
            self.menu = wx.Menu()
            self.showhide_item = utils.CreateMenuItem(
                self.menu, '&show or hide Queriet', self.MUI.showhide)
            self.openSite_item = utils.CreateMenuItem(
                self.menu, 'Open the Queriet &website', self.MUI.OpenSite)
            self.options_item = utils.CreateMenuItem(
                self.menu, '&Options...', self.MUI.ShowOptions)
            self.about = utils.CreateMenuItem(
                self.menu, 'About...', self.MUI.OnAbout, id=wx.ID_ABOUT)
            self.menu.AppendSeparator()
            self.exit_item = utils.CreateMenuItem(
                self.menu, 'e&xit', self.MUI.controller.Close, id=wx.ID_EXIT)
            self.log.debug("Done!")
        except:
    ...
    def on_left_down(self, event):
        ...
        self.MUI.showhide(
            None)  # it expects to be passed an event object, so we use none
