...
class KeybindingEditor(OptionsPanel):
    ...
    def setup(self):
        ...
        for item in [
                self.ControlCheckbox,
                self.WinCheckbox,
                self.AltCheckbox,
                self.ShiftCheckbox]:
            item.Bind(wx.EVT_CHECKBOX, self.on_binding_updated)
        # We have to bind the text field event separately because the event it
        # emits (as well as the one we're looking for in the text field's case)
        # is different from that of the checkbox
    ...
    def on_list_change(self, e):
        ...
        self.set_binding(self.config[self.OptionsListbox.GetString(
            self.OptionsListbox.GetSelection())])
    ...
    def on_binding_updated(self, e):
        ...
        self.config[self.OptionsListbox.GetString(
            self.OptionsListbox.GetSelection())] = f
