...
class OSXKeyboardHandler(KeyboardHandler):
    ...
    def register_key(self, key, function):
        ...
        key_id = RegisterEventHotKey(
            k, m, (0, 0), GetApplicationEventTarget(), 0)
...
class KeyboardCapturingNSApplication(NSApplication):
    ...
    def sendEvent_(self, theEvent):
        if theEvent.type() == NSSystemDefined and theEvent.subtype(
        ) == kEventHotKeyPressedSubtype:
            self.activateIgnoringOtherApps_(True)
            NSRunAlertPanel(u'Hot Key Pressed',
                            u'Hot Key Pressed', None, None, None)
