...
class Controller(object):
    ...
    def SetupPlugins(self):
        ...
        for plugin in self.pm.getAllPlugins():
            self.log.debug("Activating plugin %s, version %s, by %s" %
    ...
    def BuildPluginList(self):
        ...
        if not self.pm:
            self.log.warning(
                "Tried to build a list of plugins, but no plugin manager object found, obtaining one.")
            self.SetupPlugins()
        if self.pm:  # we have a plugin manager object
            ...
            for plugin in self.pm.getAllPlugins():
                ...
                # This, however, is just an abstraction over the yapsy layer.
                # We can use the iteration "plugin" object, which is really a
                # yapsy info object, to get variable plugin info, like name,
                # author, version, etc
                self.plugins[plugin] = plugin.plugin_object
                self.log.debug("Added plugin %s " % (plugin.name))
                # Note that plugins need to be passed a reffrence to the main
                # controller before setup, as the 'setup' method is where they
                # should be defining their panels, which need a parrent,
                # which... They get from controller.ui.panel.
                try:
                    plugin.plugin_object.SetController(self)
                except:
                    self.log.exception(
                        "Unable to pass a controller instance to plugin '%s'" %
                        (plugin.name))
                try:
                    plugin.plugin_object.setup()
                except:
                    self.log.exception(
                        "Error when setting up plugin '%s'" % (plugin.name))
            self.log.debug(
                "%d total plugins added to plugins dictionary." %
                (len(
                    self.plugins)))
            if self.ui:
                # Add all the plugins in the controller's 'plugins' dictionary
                # to the user interface
                self.ui.AddPluginsToList()
            else:
                self.log.warning(
                    "Built plugins list, but no UI object was found, so not adding them to the UI listbox.")
    ...
    def SetupKeyhooks(self):
        ...
        self.RegisterKey(self.config['keybindings'][
                         'showhide'], self.ui.showhide)
    ...
    def ShutdownPlugins(self):
        ...
        for plugin_info, plugin_object in self.plugins.iteritems():
            try:
                ...
                self.log.exception(
                    "Error deactivating plugin '%s'" % (plugin_info.name))
    ...
    def run(self):
        ...
        if self.ui:
            ...
            self.log.error(
                "No user interface found, can not start app main loop!")
