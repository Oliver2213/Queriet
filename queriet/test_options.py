...
##########
#The classes for the options dialog
##########

class Options(wx.Dialog):
	def __init__(self,config, parent=None, title="options"):
	...
	def CloseDialog(self, e):
		#I shouldn't have to destroy the panels or anything
...
class MainFrame(wx.Frame):
	...
	def setup(self):
		panel = wx.Panel(self)
		showbutton = wx.Button(panel, label="Show Dialog")
		showbutton.Bind(wx.EVT_BUTTON, self.ShowDialog)
		exitbutton = wx.Button(panel, label = "Exit")
		exitbutton.Bind(wx.EVT_BUTTON, self.DoClose)
		self.Bind(wx.EVT_CLOSE, self.DoClose)
		self.config = {}
		self.config['general'] = {'open_on_startup' : True, 'exit_to_tray' : True, 'ask_to_exit' : True}
		self.config['logging'] = {'log_level' : 'debug', 'format' : '%(asctime)s - %(name)s - %(levelname)s: %(message)s'}

	def ShowDialog(self, e):
		options = Options(self.config, parent=self)
		options.ShowModal()
