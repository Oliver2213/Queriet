...
def GetConfig(specpath, configpath):
    ...
    try:
        spec = ConfigObj(specpath, encoding='UTF8',
                         list_values=False, interpolation=False, _inspec=True)
    ...
    if valid:
        # Config is valid, write it back to disk to make sure it has all values
